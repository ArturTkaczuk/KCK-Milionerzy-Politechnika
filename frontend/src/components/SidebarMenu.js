    ...
    return (
        <Offcanvas show={show} onHide={handleClose} placement="end">
            ...
            <Offcanvas.Body>
                ...
                >
                    <h5 className="m-0">Ranking graczy</h5>
                    <span>▼</span>
                </div>

                {showLeaderboard && (
                    <div className="mb-4">
                        {loading && <p className="text-center text-muted">Ładowanie...</p>}
                        {!loading && (
                                ...
                                {leaderboard.map((user, idx) => (
                                    ...
                                    >
                                        ...
                                        <Badge bg="primary" pill>
                                            {user.total_score} pkt
                                        </Badge>
                                    </ListGroup.Item>
                                ))}
                                {leaderboard.length === 0 && <p className="text-muted text-center">Brak wyników. Zagraj w grę!</p>}
                            </ListGroup>
                        )}
                    </div>
                )}
    ...
    return (
        <div>
            <div
                ...
                style={{ cursor: 'pointer' }}
            >
                <h5 className="m-0">Twoja Historia Gier</h5>
                <span>▼</span>
            </div>
            ...
            {showHistory && (
                <div>
                    {loading && <p className="text-center text-muted">Ładowanie...</p>}
                    {!loading && games.length === 0 && <p className="text-muted text-center">Brak rozegranych gier.</p>}
                            ...
                            <div key={game.id}>
                                ...
                                {selectedGameId === game.id && (
                                    <div className="bg-white border p-3 border-top-0 mb-2">
                                        {detailsLoading && <p className="text-center">Ładowanie szczegółów...</p>}
                                        {!detailsLoading && gameDetails && (
                                            <div>
                                                {gameDetails.length === 0 && <p className="text-muted">Brak szczegółów dla tej gry.</p>}
                                                {gameDetails.map((q, i) => {
                                                    ...
                                                    return (
                                                        <div key={q.id} className={`p-2 mb-2 border rounded ${isCorrect ? 'border-success bg-opacity-10 bg-success' : 'border-danger bg-opacity-10 bg-danger'}`}>
                                                            <strong>P{i + 1}:</strong> {q.content}<br />
                                                            <small>Ty: <span className="fw-bold">{q.selected_answer}</span> | Poprawna: <span className="fw-bold">{q.correct_answer}</span></small>
                                                        </div>
                                                    );
                                                })}
                                            </div>
